...
calling for 42,706 markers. Quality control was applied by removing SNP markers with minor allele frequency larger lower
 ...
 \begin{table}[!htb]
  \caption{Number of available observations and average grain yield, by environmental condition}
  \begin{center}
  \begin{tabular}{cccccc}
  \hline
  \multicolumn{2}{c}{\textbf{Planting conditions}}  & \textbf{Number of} & \textbf{} & \textbf{} & \textbf{Average (sd)} \\
...
The above described data will be used to implement sparse family indices methodology for grain yield. Analyses will be implemented in \texttt{R} software using the \texttt{SFSI} R-package. Regression coefficients (in \eqref{eqn:SFI}) are separately calculated for the $i^\text{th}$ individual ($i=1,...,n_{TST}$) using $\textbf{G}_{TRN,TST(i)}$ and $\textbf{G}_{TRN,TRN}+\lambda_0 \textbf{I}$ as inputs as in \citet{Lopez-Cruz2019} for a penalized selection index.

\subsection{Data preparation}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Both phenotypic and marker data can be downloaded from CIMMYT's repository at the site \url{http://genomics.cimmyt.org/wheat_50k/PG}. The file \path{G80_42706_29489_correctedgid.RData} contains the marker information and the csv file \path{Blups_condition_group_random.csv} contains the adjusted phenotypes for all environmental conditions. \textbf{Box 1} below shows how to prepare data only for the environment \textit{EHT\_OBR} containing $n=2,040$ genotypes, and hereinafter analyses will be with reference to this environment.
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.75\textwidth]{FigureSFI_1}
...
\begin{SCfigure}
  \centering
  \includegraphics[width=0.55\textwidth]{FigureSFI_2}
...
\begin{figure}[htb]
  \centering
  \includegraphics[width=0.67\textwidth]{FigureSFI_3}
