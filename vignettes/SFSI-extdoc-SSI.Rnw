...
Code in \textbf{Box 5a} can be used to estimate regression coefficients using training data within each partition following equations \eqref{eqn:betaCSI} (for the CSI), and \eqref{eqn:betaPCSI} (with different number of PCs for the PC-SI). Coefficients for L1-PSI are estimated as in expression \eqref{eqn:PSI} whose solutions are found using the \texttt{lars2} function from the \texttt{SFSI} R-package. This function implements the LARS algorithm that gives solutions for an entire path of values of parameter $\lambda$. 
...
In the above code, the \texttt{solveEN} function included in the \texttt{SFSI} R-package was used for convenience to calculate the regression coefficients. \texttt{solveEN} function will yield solutions for a given grid of values of $\lambda$ (covering all the space of possible values) rather than all the $2,250$ solutions for the entire $\lambda$ path  provided by \texttt{lars2} function.
